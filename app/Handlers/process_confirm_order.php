...
if($database->rowCount() > 0){
       $database->DBQuery("UPDATE `orders` SET `order_status`='Confirmed' WHERE `order_id` = ? AND `user_id` = ?", [$identifier, $_SESSION['uid']]);
       $functions->toast_message("Order successfully confirmed.", "success", "yes", "");
}else{
